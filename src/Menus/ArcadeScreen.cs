...
{
    internal class ArcadeScreen : SelectScreen
    {
        public ArcadeScreen(MenuSystem screensystem, TextSection textsection, string spritepath, string animationpath, string soundpath)
        ...
        {
            if (m_isdone) return;
            if (m_stageselected == false || m_p1info.IsSelected == false) return;
            ...
            var init = new Combat.EngineInitialization(CombatMode.Versus, p1.Profile, m_p1info.PaletteIndex, PlayerProfiles.First().Profile, 0, stage);
