...
#warning Hack
			...
			if (@operator == Operator.Equals || @operator == Operator.NotEquals)
			{
				...
				if (rangenode != null)
				{
					Node newnode = new Node(new Token("ProjGuarded", new Tokenizing.CustomFunctionData("ProjGuarded", "ProjGuarded_", typeof(ProjGuarded_))));
